...
export function StateBadge({ state }: { state: number }) {
  return (
    <span className={`rounded border px-2 py-1 text-xs font-black ${stateClass[state] || "border-zinc-500/30 bg-zinc-500/10 text-zinc-300"}`}>
...
export function SubmissionBadge({ status }: { status: number }) {
  return (
    <span
      className={`rounded border px-2 py-1 text-xs font-black ${submissionClass[status] || "border-zinc-500/30 bg-zinc-500/10 text-zinc-300"}`}
    >
